...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
	...
	/**
	 * 创建任务队列，返回的 LinkedBlockingQueue
	 */
	@Deprecated
	...
	protected void interruptThread() {
		...
		// 线程不存在，则标记线程被打断（因为 EventLoop 的线程是延迟启动，所以可能线程并未创建）
	...
	protected void wakeup(boolean inEventLoop) {
		// !inEventLoop 判断是 EventLoop 线程。如果是，就意味着线程在执行中，不必唤醒。
	...
	public void execute(Runnable task) {
		...
		// 获取当前是否在 EventLoop 线程中
		boolean inEventLoop = inEventLoop();
		// 添加到队列
		addTask(task);
		if (!inEventLoop) {
			//  非 EventLoop 线程，启动EventLoop独占线程
			startThread();
			// 如果已经关闭，拒绝策略处理
			if (isShutdown()) {
